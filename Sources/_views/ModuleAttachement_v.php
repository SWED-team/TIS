...
class ModuleAttachement_v
{
    public static function editor($container, $content, $operation, $file){
        $url = '_controllers/ModuleAttachement.php?';
		if(isset($operation))
...
<form class="<?php echo $container["type"];  ?>_form form-horizontal" role="form" enctype="multipart/form-data" method="post"
    ...
    <div class="file-actual-buttons form-group">
        ...
        <div class="col-sm-5 col-xs-6  col-sm-offset-2">
            <a href="javascript:open_popup('filemanager/dialog.php?popup=1&type=2&amp;field_id=newFilePath&amp;relative_url=1')" class="btn btn-success col-md-5 btn-block  btn-xs" type="button"><i class="fa fa-plus-square"> </i> Add Image</a>
        </div>
...
<div class="module-container col-sm-<?php echo $container['cols'] * 3 ;?>">
        ...
        <div class="row">
            ...
            <div class="col-xs-6">
                <span class="pull-right">
                    <a onclick="updateModuleAttachement(<?php echo $container["id"];?>)"><i class="fa fa-pencil-square-o"></i></a>
                    <a onclick="deleteModuleAttachement(<?php echo $container["id"];?>)"><i class="fa fa-trash"></i></a>
                </span>
            </div>
        </div>
        <div class="row">
    ...
    <script>
        function updateModuleAttachement(id) {
            $.ajax({
                url: "_controllers/ModuleAttachement.php?show_editor=true",
                data: { "id": id },
                type: "post",
        ...
        function deleteModuleAttachement(id) {
            if (confirm("Do you really want to remove this module?")) {
                $.ajax({
                    url: "_controllers/ModuleAttachement.php?delete=true",
                    data: { "id": id },
                    type: "post",
                    success: function (result) {
                        $.fancybox({
